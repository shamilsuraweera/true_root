...
class _AdminStagesPageState extends ConsumerState<AdminStagesPage> {
  ...
  Future<void> _deleteStage(Stage stage) async {
    ...
    try {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Failed to delete stage')));
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return AdminPageShell(
      title: 'Manage stages',
      subtitle: 'Control the lifecycle stages and their active sequencing.',
            ...
            stagesAsync.when(
              data: (stages) {
                ...
                final filtered = query.isEmpty
                    ...
                    : stages
                          .where(
                            (stage) => stage.name.toLowerCase().contains(query),
                          )
                          .toList();
                if (filtered.isEmpty) {
                    ...
                    for (final stage in filtered)
                      Card(
                        ...
                        child: ListTile(
                          ...
                          subtitle: Text(
                            'Sequence ${stage.sequence} • ${stage.active ? 'Active' : 'Inactive'}',
                          ),
                          trailing: Wrap(
                            spacing: 8,
                            children: [
